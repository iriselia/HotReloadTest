...
namespace reload
{
  namespace runtime
  {
    ...
    std::experimental::filesystem::path RuntimeObjectSystem::ProjectSettings::ms_DefaultIntermediatePath;

    static std::experimental::filesystem::path GetIntermediateFolder(std::experimental::filesystem::path basePath_, reload::build::optimization_levels optimizationLevel_)
    {
      std::string folder;
#ifdef _DEBUG
      folder = "DEBUG_";
      ...
      std::experimental::filesystem::path runtimeFolder = basePath_ / folder;
    ...
    void RuntimeObjectSystem::StartRecompile()
    {
      ...
      if (!bHaveProjectToBuild)
      {
        if (m_pCompilerLogger)
        {
          m_pCompilerLogger->LogError("Error - could not find any files to build in any projects\n");
        }
        m_bCompiling = false;
        return;
      }
    ...
    FileSystemUtils::Path RuntimeObjectSystem::FindFile(const FileSystemUtils::Path& input)
    {
            ...
            if (depth)
            {
              ...
              if (directory.Exists())
              {
                ...
                if (foundFile.Exists())
                {
                  ...
                  if (m_pCompilerLogger)
                  {
                    m_pCompilerLogger->LogInfo("Found Directory Mapping: %s to %s\n", requestedDirectory.c_str(), directory.c_str());
                  }
                  bFoundMapping = true;
                }
              }
      ...
      if (!foundFile.Exists())
      {
        if (m_pCompilerLogger)
        {
          m_pCompilerLogger->LogWarning("Could not find Directory Mapping for: %s\n", input.c_str());
        }
    ...
    bool RuntimeObjectSystem::TestBuildCallback(const char* file, TestBuildResult type)
    {
      switch (type)
      {
        case TESTBUILDRRESULT_SUCCESS:            // SUCCESS, yay!
          if (m_pCompilerLogger)
          {
            m_pCompilerLogger->LogInfo("TESTBUILDRRESULT_SUCCESS: %s\n", file);
          }
          break;
        case TESTBUILDRRESULT_NO_FILES_TO_BUILD:  // file registration error or no runtime files of this type
          if (m_pCompilerLogger)
          {
            m_pCompilerLogger->LogWarning("TESTBUILDRRESULT_NO_FILES_TO_BUILD\n");
          }
          break;
        case TESTBUILDRRESULT_BUILD_FILE_GONE:    // the file is no longer present
          if (m_pCompilerLogger)
          {
            m_pCompilerLogger->LogError("TESTBUILDRRESULT_BUILD_FILE_GONE: %s\n", file);
          }
          break;
      ...
      ITestBuildNotifier* callback, bool bTestFileTracking)
    {
      ...
      if (pLog)
      {
        pLog->LogInfo("Testing change to file: %s\n", file.c_str());
      }
      ...
      if (file.Exists())
      {
        ...
        if (pRTObjSys->GetIsCompiling())
        {
          ...
          {
            if (!callback->TestBuildCallback(file.c_str(), TESTBUILDRRESULT_SUCCESS))
            {
              return -0xD1E;
            }
            return 0;
          ...
          else
          {
            ...
            {
              if (!callback->TestBuildCallback(file.c_str(), TESTBUILDRRESULT_BUILD_FAILED))
              {
                return -numErrors;
              }
            }
            else
            {
              ...
              if (!callback->TestBuildCallback(file.c_str(), TESTBUILDRRESULT_OBJECT_SWAP_FAIL))
              {
                return -numErrors;
              }
            }
        ...
        {
          ++numErrors;
          if (!callback->TestBuildCallback(file.c_str(), TESTBUILDRRESULT_BUILD_NOT_STARTED))
          {
            return -numErrors;
          }
        }
      ...
      {
        ++numErrors;
        if (!callback->TestBuildCallback(file.c_str(), TESTBUILDRRESULT_BUILD_FILE_GONE))
        {
          return -numErrors;
        }
      }
    ...
    int RuntimeObjectSystem::TestBuildAllRuntimeSourceFiles(ITestBuildNotifier* callback, bool bTestFileTracking)
    {
      if (m_pCompilerLogger)
      {
        m_pCompilerLogger->LogInfo("TestBuildAllRuntimeSourceFiles Starting\n");
      }
      ...
      if (0 == numErrors)
      {
        if (m_pCompilerLogger)
        {
          m_pCompilerLogger->LogInfo("All Tests Passed\n");
        }
      }
      else
      {
        if (m_pCompilerLogger)
        {
          m_pCompilerLogger->LogError("Tests Failed: %d\n", numErrors);
        }
      }
      return numErrors;
    }
