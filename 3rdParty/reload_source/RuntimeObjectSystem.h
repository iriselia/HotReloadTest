...
namespace reload
{
  ...
  namespace runtime
  {
    ...
    public:
      ...
      virtual void SetOptimizationLevel(reload::build::optimization_levels optimizationLevel_, unsigned short projectId_ = 0);
      virtual reload::build::optimization_levels GetOptimizationLevel(unsigned short projectId_ = 0);
    ...
    private:
      ...
      bool					m_bAutoCompile;
      std::experimental::filesystem::path   m_CurrentlyCompilingModuleName;
      ...
      struct ProjectSettings
      {
        ProjectSettings()
        {
          m_CompilerOptions.optimizationLevel = reload::build::optimization_levels::optimization_level_default;
          m_CompilerOptions.base_intermediate_path = ms_DefaultIntermediatePath;
        }

        reload::build::compiler_options						m_CompilerOptions;
        ...
        std::vector<build_tool::file_to_build> m_BuildFileList;
        std::vector<build_tool::file_to_build> m_PendingBuildFileList; // if a compile is already underway, store files here.

        static std::experimental::filesystem::path		ms_DefaultIntermediatePath;
      };
